...
<template>
  <div>
    ...
    <header
      class="flex overflow-hidden items-center justify-between px-6 py-4 bg-white border-b-4 border-indigo-600"
    >
            ...
            <table class="min-w-full">
              ...
              <tbody class="bg-white">
                <tr v-for="question of questions" :key="question.id">
                  <div v-if="isClick" class="flex items-center p-5 gap-2">
                    <textarea
                      class="bg-gray-200 border rounded"
                      v-if="question.id"
                      rows="2"
                      type="text"
                      @change="editQuestionData"
                    />
                    <button @click="editQuestions(question.id)">Edit</button>
                  </div>
                  ...
                  >
                    ...
                    <button
                      type="button"
                      class="text--600 overflow-hidden hover:text-indigo-900"
                    >
                      <modal :id="question.id" :token="token"></modal>
                    </button>
                  </td>
                  <td
                    class="px-6 py-4 text-sm leading-5 text-gray-500 border-b border-gray-200 whitespace-nowrap"
...
export default {
  ...
  mounted() {
    ...
    console.log("user type", parsedata.userType);
    if (parsedata.userType == "USER") {
      this.$router.push("/score");
    }
    this.user = parsedata;
    console.log("this is user locsl", localStorage.getItem("user"));
  },
  ...
  methods: {
    ...
    editQuestionData(e) {
      this.questionData = e.target.value;
    },
    getAnswers(id) {
      console.log("id", id);
